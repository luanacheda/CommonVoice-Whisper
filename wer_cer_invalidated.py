...
whisper_df = pd.read_csv('transcriptions_large_1000_invalidated.csv', encoding='utf-8')
...
output_df.to_csv('sentences_invalidated_whisper_intermediate.csv', index=False, encoding='utf-8')


# Load the CSV file into a DataFrame
df = pd.read_csv('sentences_invalidated_whisper_intermediate.csv', encoding='utf-8')
...
def tokenize_sentence_wer(sentence):
    ...
    sentence_tok_hyphen = re.sub("’", "'", sentence_tok_perc)
    sentence_tok = re.sub(r"[^\w+\s'-]", "", sentence_tok_hyphen)
...
os.remove('sentences_invalidated_whisper_intermediate.csv')
